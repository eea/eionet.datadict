...
public abstract class XForm implements XFormIF {
    ...
    private void writeInstance(String lead) throws Exception {
        writer.println(lead
                + "<!-- Original instance in Data Dictinary (not used anymore - use default instance value with context parameter $instance) -->");
        writer.println(lead + "<!--  <f:instance id=\"original\" src=\"" + instance + "\"/>  -->");
    }

    private void writeControlsLabel(String lead) throws Exception {
        writer.println(lead + "<f:label>" + escapeXml(controlsLabel) + "</f:label>");
    }
    ...
    protected void writeRegularBinds(String lead) throws Exception {
        ...
        for (int i = 0; i < binds.size(); i++) {
            ...
            if (constraint != null) {
                buf.append(" constraint=\"").append(constraint).append("\"");
            }
    ...
    private void writeControls(String lead) throws Exception {
        ...
        for (int i = 0; i < controls.size(); i++) {
            ...
            // write label
            if (label != null) {
                buf = new StringBuffer("<f:label>").append(escapeXml(label)).append("</f:label>");
                writer.println(lead + "\t" + buf.toString());
            }

            // write hint
            if (hint != null) {
                buf = new StringBuffer("<f:hint>").append(escapeXml(hint)).append("</f:hint>");
                writer.println(lead + "\t" + buf.toString());
            }

            // write alert
            if (alert != null) {
                buf = new StringBuffer("<f:alert>").append(escapeXml(alert)).append("</f:alert>");
                writer.println(lead + "\t" + buf.toString());
            }
    ...
    private void writeSelectItem(FixedValue fxv, String lead) throws Exception {
        ...
        writer.println(lead + "\t<f:label>" + escapeXml(label) + "</f:label>");
        writer.println(lead + "\t<f:value>" + escapeXml(value) + "</f:value>");
        writer.println(lead + "</f:item>");
    }
     ...
     * (non-Javadoc)
     *
     * @see eionet.meta.exports.xforms.XFormIF#write(java.lang.String)
     */
    @Override
    public abstract void write(String objID) throws Exception;

    /**
     * Convenience method for XML-escaping the given string.
     *
