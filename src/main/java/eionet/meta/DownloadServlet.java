 ...
 * {@link #processRequest(HttpServletRequest, HttpServletResponse, boolean)} method. All POST, GET and HEAD requests are delegated
 * to this method.
 *
 * Requested files are identified through the HTTP request's path info, which is physically resolved relatively to the root
 * directory of the DD file store (see {@link FileStore}).
 *
...
public class DownloadServlet extends HttpServlet {
    ...
    ServletException {
        ...
        // Get requested file's path.
        String requestURI = request.getRequestURI();
        String contextPath = request.getContextPath();
        int contextPathLength = contextPath.length();
        String filePath = contextPathLength < requestURI.length() ? requestURI.substring(contextPathLength) : null;

        // Handle the case where no file path was found in the request
        if (filePath == null) {
        ...
        String contentType = getServletContext().getMimeType(fileName.toLowerCase());
