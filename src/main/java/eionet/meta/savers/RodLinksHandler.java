...
public class RodLinksHandler extends BaseHandler {
    ...
    private void addRodLinks(String dstID) throws Exception {
        ...
        try {
            ...
            // Determine if an activity by the given ID exists in ROD_ACTIVITIES.
            // If not, add it there. Otherwise update it there.
            ...
            stmt = SQL.preparedStatement(buf.toString(), inParams, conn);
            rs = stmt.executeQuery();
            boolean activityExists = rs.next() && rs.getInt(1) > 0;
            ...
            // If new activity, add it, otherwise update.
            if (!activityExists) {
            ...
            // And now link the newly inserted/updated activity to this dataset.
            inParams = new INParameters();
