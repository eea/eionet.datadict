...
@Component("vocabularyCheckOutTask")
@Scope("prototype")
public class VocabularyCheckOutTask implements AsyncTask {
    ...
    @Autowired
    public VocabularyCheckOutTask(IVocabularyService vocabularyService) {
    ...
    public String getDisplayName() {
        return String.format("Checking out vocabulary %s/%s", this.getVocabularySetIdentifier(), this.getVocabularyIdentifier());
    }

    @Override
    public String composeResultUrl(String taskId, Object result) {
        return String.format("/vocabulary/%s/%s?vocabularyFolder.workingCopy=true", this.getVocabularySetIdentifier(), this.getVocabularyIdentifier());
    }

    @Override
    public Object call() throws Exception {
        Thread.currentThread().setName("VOCABULARY-CHECK-OUT");
        vocabularyService.checkOutVocabularyFolder(this.getVocabularyId(), this.getUserName());
