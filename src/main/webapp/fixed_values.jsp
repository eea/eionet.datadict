    ...
    <div id="workarea">
        ...
        <form id="form1" method="post" action="fixed_values.jsp">
                ...
                <table class="datatable">
                    ...
                    for (int i=0; i<fixedValues.size(); i++){
                        ...
                        <tr <% if (i % 2 != 0) %> class="zebraeven" <% else %> class="zebraodd" <%; %>>
                            <td>
                                <%=spaces%>
                                <a href="fixed_value.jsp?fxv_id=<%=fxvID%>&amp;mode=<%=mode%>&amp;delem_id=<%=delem_id%>&amp;delem_name=<%=Util.processForDisplay(delem_name)%>&amp;parent_type=<%=typeParam%>">
                    ...
                    for (int i=0; i<fixedValues.size(); i++){
                        ...
                        <tr <%=trStyle%>>
                            ...
                            <td valign="bottom" align="left" style="padding-left:5;padding-right:10">
                                <b><a href="fixed_value.jsp?fxv_id=<%=fxvID%>&amp;mode=edit&amp;delem_id=<%=delem_id%>&amp;delem_name=<%=Util.processForDisplay(delem_name)%>&amp;parent_type=<%=typeParam%>">
                                    <%=Util.processForDisplay(value)%>
                                </a></b>&nbsp;
                            </td>
            ...
            <div style="display:none">
                ...
                <input type="hidden" name="delem_name" value="<%=Util.processForDisplay(delem_name)%>"/>
