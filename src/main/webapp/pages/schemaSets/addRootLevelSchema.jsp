    ...
    <stripes:form method="post" beanclass="${actionBean['class'].name}">
        <div id="outerframe">
            <table class="datatable">
                ...
                <tr>
                    <th scope="row" class="scope-row simple_attr_title">
                        File name
                        <a class="helpButton" href="/help.jsp?screen=schema&amp;area=filename"></a>
                    </th>
                    <td class="simple_attr_help">
                        <img style="border:0" src="<%=request.getContextPath()%>/images/mandatory.gif" width="16" height="16" alt=""/>
                    </td>
                    <td class="simple_attr_value">
                        <stripes:file name="uploadedFile" size="40"/>
                    </td>
                </tr>
                            ...
                            <td style="word-wrap:break-word;wrap-option:emergency" class="simple_attr_value">
                                <c:if test="${!attribute.multipleValuesAllowed || attribute.displayType=='image'}">
                                    ...
                                    <c:if test="${attribute.displayType=='image'}">
                                        <span class="barfont">
                                            <a href="${actionBean.contextPath}/imgattr.jsp?obj_id=${actionBean.schema.id}&amp;obj_type=<%=DElemAttribute.ParentType.SCHEMA%>&amp;attr_id=${attribute.ID}&amp;obj_name=${actionBean.schemaSet.identifier}&amp;attr_name=${attribute.shortName}">
            ...
            <fieldset style="display:none">
                <stripes:hidden name="schema.workingCopy" value="true"/>
            </fieldset>
        </div>
    </stripes:form>
