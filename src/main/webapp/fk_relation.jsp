...
if (relID == null || relID.isEmpty()) {
    request.setAttribute("DD_ERR_MSG", "Missing foreign key relation id.");
    request.getRequestDispatcher("error.jsp").forward(request, response);
    return;
}

if (NumberUtils.toInt(relID) <= 0) {
    request.setAttribute("DD_ERR_MSG", "Invalid foreign key relation id.");
    request.getRequestDispatcher("error.jsp").forward(request, response);
    return;
}
...
<head>
    ...
    <script type="text/javascript">
        ...
        function submitForm(mode) {
            if (mode === "delete") {
                const b = confirm("This relation will be deleted! Click OK, if you want to continue. Otherwise click Cancel.");
                if (!b) return;
            }
    ...
    <div id="workarea">
        <form id="form1" method="post" action="fk_relation.jsp">
            ...
            <p>
                ...
                <em><%=Util.processForDisplay((String) fkRel.get("a_name"))%>
                </em> and
                <em><%=Util.processForDisplay((String) fkRel.get("b_name"))%>
                </em>.<br/>
            ...
            <table cellspacing="0" cellpadding="0" class="datatable">
                ...
                <tr>
                    ...
                    <td><em><%=Util.processForDisplay((String) fkRel.get("a_name"))%>
                    </em></td>
                </tr>

                <tr>
                    <th scope="row">Element B</th>
                    <td><em><%=Util.processForDisplay((String) fkRel.get("b_name"))%>
                    </em></td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <select name="a_cardin">
                            ...
                            <option <%=selected%>
                                    value="<%=Util.processForDisplay(cardin, true)%>"><%=Util.processForDisplay(cardin)%>
                            </option>
                            <%
                                }
                            %>
                ...
                <tr>
                    <td></td>
                    <td>
                        <input type="button" <%=disabled%> class="mediumbuttonb" value="Save"
                               onclick="submitForm('edit')"/>&nbsp;&nbsp;
                        <input type="button" <%=disabled%> class="mediumbuttonb" value="Delete"
