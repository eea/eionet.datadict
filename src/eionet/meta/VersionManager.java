...
public class VersionManager{
    ...
    */
    public VersionManager(){
        ...
        // init registration statuses
        regStatuses.put("Card", "true");
        regStatuses.put("Draft", "false");
    ...
    */
     ...
     * object to copy (e.g. "DATAELEM_ID=123").
     * 
     * @param   dstGen    table name and preset values.
     * @param   srcConstraint    the constraint.
     * @return  id of the working copy.
     * @exception   SQLException
     */
    private String copy(SQLGenerator dstGen, String srcConstraint)
        throws SQLException{
        ...
        if (latestElm!=null){
            if (newVersion.equals(elm.getVersion())){
                ...
                DataElementHandler delemHandler =
                    new DataElementHandler(conn, params, null);
        ...
        if (latestTbl!=null){
            if (newVersion.equals(tbl.getVersion())){
                ...
                DsTableHandler tblHandler =
                    new DsTableHandler(conn, params, null);
		...
		if (latestDst!=null){
			if (newVersion.equals(dst.getVersion())){
				...
				DatasetHandler dstHandler =
					new DatasetHandler(conn, params, null);
    ...
    */
    private String copyTbl(String tblID) throws Exception {
        return copyTbl(tblID, true);
    }
    
    /**
    *
    */
    private String copyTbl(String tblID, boolean wrkCopy) throws Exception {
        
        // find the table to copy, create a new row in DS_TABLE,
        // get lastInsertID
        SQLGenerator gen = new SQLGenerator();
        gen.setTable("DS_TABLE");
        ...
        // if the copy is to be a working copy, set the credentials
        if (wrkCopy){
            if (user==null || !user.isAuthentic())
                throw new Exception("Attempt to create a working copy " +
                                        "by an unauthorized user!");
            String userName = user.getUserName();
            
            // set the copy's check-out credentials
            gen.clear();
            gen.setTable("DS_TABLE");
            gen.setField("WORKING_COPY", "Y");
            gen.setField("WORKING_USER", userName);
            String q = gen.updateStatement() + " where TABLE_ID=" + newID;
            conn.createStatement().executeUpdate(q);
            
            // set the original's WORKING_USER
            gen.removeField("WORKING_COPY");
            gen.setField("WORKING_USER", userName);
            q = gen.updateStatement() + " where TABLE_ID=" + tblID;
            conn.createStatement().executeUpdate(q);
        }
    ...
    */
    private String copyDst(String dstID) throws Exception {
        return copyDst(dstID, true);
    }
    
    /**
    *
    */
    private String copyDst(String dstID, boolean wrkCopy) throws Exception {
        
        // find the dataset to copy, create a new row in DATASET,
        // get lastInsertID
        SQLGenerator gen = new SQLGenerator();
        gen.setTable("DATASET");
        ...
        if (wrkCopy){
            ...
            gen.setField("WORKING_COPY", "Y");
            gen.setField("WORKING_USER", userName);
            String q = gen.updateStatement() + " where DATASET_ID=" + newID;
            conn.createStatement().executeUpdate(q);
            
            // set the original's WORKING_USER
            gen.removeField("WORKING_COPY");
            gen.setField("WORKING_USER", userName);
            q = gen.updateStatement() + " where DATASET_ID=" + dstID;
            conn.createStatement().executeUpdate(q);
        }
    ...
    */
    public void deleteDst(String dstID) throws Exception{
        ...
        // make sure that this is the latest version of a dataset
        // with such logical ID
        String latestID = getLatestDstID(dst);
        if (latestID!=null && !latestID.equals(dst))
            throw new Exception("Cannot delete an intermediate version!");
        
        // delete (without any versioning logic) all tables that belong
        // only into this dataset
    }
