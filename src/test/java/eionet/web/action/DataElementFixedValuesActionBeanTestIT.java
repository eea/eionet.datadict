 ...
 */
public class DataElementFixedValuesActionBeanTestIT {
    ...
    public void testFailToViewSingleValueBecauseOfOwnerNotFound() throws Exception {
        ...
        final boolean isEditView = false;
        MockRoundtrip trip = this.prepareRoundTrip(ownerId, valueId);
        when(controller.getSingleValueModel(eq(ownerId), eq(valueId))).thenThrow(ResourceNotFoundException.class);
        trip.execute("view");
    ...
    public void testFailToAddValueBecauseOfOwnerNotFound() throws Exception {
        ...
        final boolean isEditView = true;
        MockRoundtrip trip = this.prepareRoundTrip(ownerId);
        when(controller.getEditableOwnerDataElement(any(AppContextProvider.class), eq(ownerId))).thenThrow(ResourceNotFoundException.class);
        trip.execute("add");
        verify(controller, times(1)).getEditableOwnerDataElement(any(AppContextProvider.class), eq(ownerId));
        verify(viewModelBuilder, times(0)).buildFromOwner(any(DataElement.class), eq(isEditView));
        verify(errorPageService, times(1)).createErrorResolution(eq(ErrorActionBean.ErrorType.NOT_FOUND_404), any(String.class));
    ...
    public void testFailToAddValueBecauseOfNonOwner() throws Exception {
        ...
        final boolean isEditView = true;
        MockRoundtrip trip = this.prepareRoundTrip(ownerId);
        when(controller.getEditableOwnerDataElement(any(AppContextProvider.class), eq(ownerId))).thenThrow(ConflictException.class);
        trip.execute("add");
        verify(controller, times(1)).getEditableOwnerDataElement(any(AppContextProvider.class), eq(ownerId));
        verify(viewModelBuilder, times(0)).buildFromOwner(any(DataElement.class), eq(isEditView));
        verify(errorPageService, times(1)).createErrorResolution(eq(ErrorActionBean.ErrorType.CONFLICT), any(String.class));
