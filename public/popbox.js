...
function _ScanForLinkedImages(images)
{
	for (var x = 0; x < images.length; x++)
	{
		for (class_name in _popclasses) {
			var parent = images[x].parentNode;
			if (hasClassName(images[x], class_name) && parent.tagName == 'A') {
				if (images[x].id == "")
				...
				if (pbSrc[images[x].id] == null)
				{
					...
					pbSrc[images[x].id].src = images[x].parentNode.getAttribute('href');
					images[x].parentNode.onclick = function() { this.blur(); return false; };
					var popattrs = _popclasses[class_name];

					var imgevents = ['onmouseover','onmouseout','onclick','ondblclick'];
					for (var inx = 0; inx < imgevents.length; inx++)
					{
						if (popattrs[imgevents[inx]] != "undefined" && popattrs[imgevents[inx]] != null) {
							images[x][imgevents[inx]] = popattrs[imgevents[inx]];
						}
					}

					var imgattrs = ['pbCaption','pbPopImage', 'pbPopText',' pbRevertImage', 'pbRevertText',
					 'pbShowCaption','pbShowPopBar','pbShowPopImage', 'pbShowPopText',
					 'pbShowRevertBar','pbShowRevertImage','pbShowRevertText'];
					for (var inx = 0; inx < imgattrs.length; inx++)
					{
						if (popattrs[imgattrs[inx]] != null) {
							images[x].setAttribute(imgattrs[inx], popattrs[imgattrs[inx]]);
						}
					}
				}
				//var grandparent = images[x].parentNode.parentNode;
				//grandparent.replaceChild(images[x],images[x].parentNode);
			}
		}
	}
}
...
function CreatePbBar(obj, pbShowBar, pbShowText, pbShowImage, pbText, pbImage, pbBarAbove, isRevert, pbCaption)
{
	...
	if (pbCaption != null && pbCaption != "")
	{
		...
		_AddCaptionText(divCapTrans, divCapText, pbCaption);
	}
...
function _AddCaptionText(divCapTrans, divCapText, caption)
{
...
function ResizeCaption(divCapTrans, divCapText, height, caption)
{
	...
	else
	{
		...
		if (h == height)
		{
			...
			_AddCaptionText(divCapTrans, divCapText, caption);
		}
		else
		{
			setTimeout("ResizeCaption(\"" + divCapTrans.id + "\",\"" + divCapText.id + "\"," + height + ",\"" + caption + "\")", 10);
...
var _popclasses = new Array();

function PopRegisterClass(classid, obj)
{
	_popclasses[classid] = obj;
}
