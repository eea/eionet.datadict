...
pipeline {
  ...
  stages {
    ...
    stage('Integration Tests') {
      ...
      steps {
        // IMPORTANT:
        // - Do NOT start MySQL yourself.
        // - Do NOT override DB URL/user/pass here.
        //   The pom's docker-maven-plugin spins up mysql:5.7 and exposes a mapped port.
        // - The tests expect env=jenkins (see jenkins.properties in repo).
        sh '''
          set -eux
      ...
      post {
        always {
          // Collect IT results and publish coverage reports generated by jacoco goals bound in pom
          ...
          // These HTML reports are created by jacoco plugin executions (unit/it/merged)
